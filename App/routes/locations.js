    ...
    if(req.session.passport == undefined){
        res.redirect('login');
            ...
            || req.session.passport.user.id == "passenger"){
                ...
                try{
                    pool.query(sql.query.get_location, [user_email], (err, data) => {
...
router.post('/add_fav', async function(req, res, next){
    var loc_name = req.body.fav_location;
    var data1 = await pool.query(sql.query.add_general_loc, [loc_name, "king kong avenue"]);
    var data2 = await pool.query(sql.query.add_fav_loc, [user_email, loc_name]);
    res.redirect('./');
})
